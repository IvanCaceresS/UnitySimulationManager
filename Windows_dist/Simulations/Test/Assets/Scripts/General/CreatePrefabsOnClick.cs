...
private void CrearEntidad(Vector3 p)
{
    ...
    if(n=="EColi")
    {
        entityManager.AddComponentData(e,new EColiComponent
        {
            TimeReference=1080f,SeparationThreshold=0.75f,MaxScale=1f,GrowthTime=0f,GrowthDuration=1080f*0.75f,TimeSinceLastDivision=0f,DivisionInterval=1080f*0.75f,HasGeneratedChild=false,Parent=Entity.Null,IsInitialCell=true,SeparationSign=0
        }
        );
...
private void AddPhysicsComponents(Entity e,string n,float s)
{
    ...
    if(n=="EColi")
    {
        c=Unity.Physics.CapsuleCollider.Create(new CapsuleGeometry
        {
